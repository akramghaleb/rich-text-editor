...
const util = {
  ...
  allowedEmptyNodeList: ".meta-component, pre, blockquote, hr, li, table, img, iframe, video, audio, canvas",
  ...
  getIncludePath: function (nameArray, extension) {
    ...
    const extRegExp = new this._window.RegExp(".+\\." + extension + "(:\\?.*|;.*)?$", "i");

    for (let c = this._document.getElementsByTagName(tagName), i = 0; i < c.length; i++) {
    ...
    if (!path) {
      throw (
        "[ExploreEditor.util.getIncludePath.fail] The ExploreEditor installation path could not be automatically detected. (name: +" +
        name +
        ", extension: " +
        extension +
        ")"
   ...
   * @param {Document|null} doc To get the CSS text of an document(core._iframeDocument). If null get the current document.
  ...
  isWysiwygDiv: function (element) {
    return (
      element &&
      element.nodeType === 1 &&
      (this.hasClass(element, "meta-wrapper-wysiwyg") || /^BODY$/i.test(element.nodeName))
    );
  },
  ...
  /**
   * @description It is judged whether it is the format element (P, DIV, H[1-6], PRE, LI | class="__meta__format__replace_xxx")
  ...
  isFormatElement: function (element) {
    return (
      ...
      (/^(P|DIV|H[1-6]|PRE|LI|TH|TD)$/i.test(element.nodeName) ||
        this.hasClass(element, "(\\s|^)__meta__format__replace_.+(\\s|$)|(\\s|^)__meta__format__free_.+(\\s|$)")) &&
  ...
  /**
   * @description It is judged whether it is the range format element. (BLOCKQUOTE, OL, UL, FIGCAPTION, TABLE, THEAD, TBODY, TR, TH, TD | class="__meta__format__range_xxx")
  ...
  isRangeFormatElement: function (element) {
    return (
      ...
      (/^(BLOCKQUOTE|OL|UL|FIGCAPTION|TABLE|THEAD|TBODY|TR|TH|TD)$/i.test(element.nodeName) ||
        this.hasClass(element, "(\\s|^)__meta__format__range_.+(\\s|$)"))
    );
  },

  /**
   * @description It is judged whether it is the closure range format element. (TH, TD | class="__meta__format__range__closure_xxx")
  ...
  isClosureRangeFormatElement: function (element) {
    return (
      element &&
      element.nodeType === 1 &&
      (/^(TH|TD)$/i.test(element.nodeName) || this.hasClass(element, "(\\s|^)__meta__format__range__closure_.+(\\s|$)"))
    );
  },

  /**
   * @description It is judged whether it is the free format element. (PRE | class="__meta__format__free_xxx")
  ...
  isFreeFormatElement: function (element) {
    return (
      element &&
      element.nodeType === 1 &&
      (/^PRE$/i.test(element.nodeName) || this.hasClass(element, "(\\s|^)__meta__format__free_.+(\\s|$)")) &&
      !this.isComponent(element) &&
      !this.isWysiwygDiv(element)
    );
  },
  ...
  /**
   * @description It is judged whether it is the closure free format element. (class="__meta__format__free__closure_xxx")
  ...
  isClosureFreeFormatElement: function (element) {
    return (
      element && element.nodeType === 1 && this.hasClass(element, "(\\s|^)__meta__format__free__closure_.+(\\s|$)")
    );
  },

  /**
   * @description It is judged whether it is the component[img, iframe, video, audio, table] cover(class="meta-component") and table, hr
   * @param {Node} element The node to check
   * @returns {Boolean}
   */
  isComponent: function (element) {
    return element && (/meta-component/.test(element.className) || /^(TABLE|HR)$/.test(element.nodeName));
  },

  /**
   * @description Checks for "__meta__uneditable" in the class list.
   * Components with class "__meta__uneditable" cannot be modified.
   * @param {Element} element The element to check
   * @returns {Boolean}
   */
  isUneditableComponent: function (element) {
    return element && this.hasClass(element, "__meta__uneditable");
  },

  /**
   * @description It is judged whether it is the component [img, iframe] cover(class="meta-component")
   * @param {Node} element The node to check
   * @returns {Boolean}
   */
  isMediaComponent: function (element) {
    return element && /meta-component/.test(element.className);
  },

  /**
   * @description It is judged whether it is the not checking node. (class="katex", "__meta__tag")
   * @param {Node} element The node to check
   * @returns {Boolean}
   */
  isNotCheckingNode: function (element) {
    return element && /katex|__meta__tag/.test(element.className);
  },
  ...
  /**
   * @description Copy and apply attributes of format tag that should be maintained. (style, class) Ignore "__meta__format__" class
   * @param {Element} originEl Origin element
   * @param {Element} copyEl Element to copy
   */
  copyFormatAttributes: function (originEl, copyEl) {
    copyEl = copyEl.cloneNode(false);
    copyEl.className = copyEl.className.replace(/(\s|^)__meta__format__[^\s]+/g, "");
  ...
  getNumber: function (text, maxDec) {
    ...
    return maxDec < 0 ? number * 1 : maxDec === 0 ? this._window.Math.round(number * 1) : (number * 1).toFixed(maxDec);
  },
  ...
  getOffset: function (element, wysiwygFrame) {
    ...
    while (offsetElement && !this.hasClass(offsetElement, "meta-container") && offsetElement !== wysiwyg) {
   ...
   * core.codeViewDisabledButtons (An array of buttons whose class name is not "meta-code-view-enabled")
   * core.resizingDisabledButtons (An array of buttons whose class name is not "meta-resizing-enabled")
  ...
  mergeSameTags: function (element, nodePathArray, onlyText) {
    ...
    if (nodePathLen) {
      offsets = _this._window.Array.apply(null, new _this._window.Array(nodePathLen)).map(
        _this._window.Number.prototype.valueOf,
        0
      );
    }
    ...
    (function recursionFunc(current, depth, depthIndex) {
      ...
      for (let i = 0, len = children.length, child, next; i < len; i++) {
        ...
        if (
          ...
          (!onlyText &&
            (_this.isTable(child) ||
              ...
              (_this.isFormatElement(child) && !_this.isFreeFormatElement(child))))
        ) {
        ...
        if (child.nodeName === next.nodeName && _this.isSameAttributes(child, next) && child.href === next.href) {
  ...
  mergeNestedTags: function (element, validation) {
    if (typeof validation === "string") {
      ...
      }.bind(new this._window.RegExp("^(" + (validation ? validation : ".+") + ")$", "i"));
    ...
    (function recursionFunc(current) {
      ...
      if (children.length === 1 && children[0].nodeName === current.nodeName && vld) {
  ...
  removeEmptyNode: function (element, notRemoveNode) {
    ...
    (function recursionFunc(current) {
      if (_this.notTextNode(current) || current === notRemoveNode || _this.isNonEditable(current)) {
        return 0;
      }
      if (
        current !== element &&
   ...
   * @private
   */
  consistencyCheckOfHTML: function (documentFragment, htmlCheckWhitelistRegExp) {
    /**
    ...
    const wrongTags = this.getListChildNodes(
      ...
      function (current) {
        ...
        if (
          ...
          (this.isFormatElement(current) || this.isRangeFormatElement(current) || this.isTextStyleElement(current)) &&
          current.childNodes.length === 0 &&
          nrtag
        ) {
  ...
  setIframeCssTags: function (options) {
    ...
    for (let f = 0, len = linkNames.length, path; f < len; f++) {
      ...
      if (!path || path.length === 0) {
        throw '[ExploreEditor.constructor.iframe.fail] The ExploreEditor CSS files installation path could not be automatically detected. Please set the option property "iframeCSSFileName" before creating editor instances.';
      }
