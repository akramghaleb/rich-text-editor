...
/**
 * @description ExploreEditor constuctor function.
 ...
 */
export default function (context, options, plugins, pluginCallButtons, responsiveButtons) {
  const _document = context.element.originElement.ownerDocument || window.document;
  ...
  const core = {
     ...
     * @private
     */
    _iframeDocument: null, // _iframeDocument
    ...
    /**
     * @description Elements and user options parameters of the ExploreEditor
     */
    context: context,
    ...
    /**
     * @description An array of buttons whose class name is not "meta-code-view-enabled"
     */
    codeViewDisabledButtons: null,

    /**
     * @description An array of buttons whose class name is not "meta-resizing-enabled"
     */
     ...
     * 1. All classes not starting with "__meta__" or "meta-" in the editor are removed.
     * 2. The style of all tags except the "span" tag is removed from the editor.
     * "managedTagsInfo" structure ex:
     * managedTagsInfo: {
     *   query: '.__meta__xxx, meta-xxx'
     *   map: {
     *     '__meta__xxx': method.bind(core),
     *     'meta-xxx': method.bind(core),
     *   }
     * }
     * @example
     ...
     * managedTagInfo() => {
     *  return {
     *    className: 'string', // Class name to identify the tag. ("__meta__xxx", "meta-xxx")
     ...
     * @property {Number} minResizingSize Minimum size of editing area when resized {Number} (.meta-wrapper-inner {min-height: 65px;} || 65)
    ...
    callPlugin: function (pluginName, callBackFunction, _target) {
      ...
      if (!this.plugins[pluginName]) {
        throw Error(
          '[ExploreEditor.core.callPlugin.fail] The called plugin does not exist or is in an invalid format. (pluginName:"' +
            pluginName +
            '")'
        );
      ...
      } else if (typeof this.targetPlugins[pluginName] === "object" && !!_target) {
        this.initMenuTarget(pluginName, _target, this.targetPlugins[pluginName]);
      }
    ...
    submenuOn: function (element) {
      ...
      const submenuName = (this.submenuName = element.getAttribute("data-command"));
    ...
    containerOn: function (element) {
      ...
      const containerName = (this.containerName = element.getAttribute("data-command"));
    ...
    setMenuPosition: function (element, menu) {
      ...
      const menuHeight_bottom =
        _window.innerHeight - (toolbarTop - scrollTop + bt + element.parentElement.offsetHeight);
    ...
    controllersOn: function () {
      ...
      for (let i = 0, arg; i < arguments.length; i++) {
        ...
        if (!util.hasClass(arg, "meta-controller")) {
        ...
        if (arg.style) {
          ...
          if (this.shadowRoot && this.shadowRootControllerEventTarget.indexOf(arg) === -1) {
    ...
    setControllerPosition: function (controller, referEl, position, addOffset) {
      ...
      const allow = util.hasClass(controller.firstElementChild, "meta-arrow") ? controller.firstElementChild : null;
    ...
    execCommand: function (command, showDefaultUI, value) {
      this._iframeDocument.execCommand(command, showDefaultUI, command === "formatBlock" ? "<" + value + ">" : value);
      // history stack
    ...
    setRange: function (startCon, startOff, endCon, endOff) {
      ...
      try {
        ...
        console.warn("[ExploreEditor.core.focus.error] " + error);
        this.nativeFocus();
        return;
      }
    ...
    getRange: function () {
      ...
      if (selection.rangeCount > 0) {
        ...
        const rightDir =
          compareValue.ancestor && (compareValue.result === 0 ? so <= eo : compareValue.result > 1 ? true : false);
    ...
    getSelection: function () {
      return this.shadowRoot && this.shadowRoot.getSelection
        ...
        : this._iframeWindow.getSelection();
    },
    ...
    getFileComponent: function (element) {
      if (!this.fileManager.queryString || !element) {
        return null;
      }

      let target, pluginName;
      if (/^FIGURE$/i.test(element.nodeName) || /meta-component/.test(element.className)) {
        target = element.querySelector(this.fileManager.queryString);
      }
      if (!target && element.nodeName && this.fileManager.regExp.test(element.nodeName)) {
        target = element;
      }
    ...
    selectComponent: function (element, pluginName) {
      if (
        util.isUneditableComponent(util.getParentElement(element, util.isComponent)) ||
    ...
    insertNode: function (oNode, afterNode, checkCharCount) {
      ...
      const isFormats =
        (!freeFormat && (util.isFormatElement(oNode) || util.isRangeFormatElement(oNode))) || util.isComponent(oNode);
        ...
        if (range.collapsed) {
          if (commonCon.nodeType === 3) {
            ...
            if (!util.isBreak(parentNode)) {
              ...
              const focusNode =
                c && c.nodeType === 3 && util.onlyZeroWidthSpace(c) && util.isBreak(c.nextSibling) ? c.nextSibling : c;
              if (focusNode) {
      ...
      try {
        ...
        if (!util.isComponent(oNode)) {
          ...
          if (oNode && oNode.nodeType === 3) {
            ...
            const previousText =
              !previous || previous.nodeType === 1 || util.onlyZeroWidthSpace(previous) ? "" : previous.textContent;
    ...
    removeNode: function () {
      ...
      let endOff = range.endOffset;
      const commonCon =
        range.commonAncestorContainer.nodeType === 3 && range.commonAncestorContainer.parentNode === startCon.parentNode
          ? startCon.parentNode
      ...
      for (
        ...
        if (util.isList(originParent)) {
          ...
          if (i === len - 1 || (nextLine && nextLine.parentNode !== originParent)) {
            ...
            if (nextList) {
              edge.removeArray[edge.removeArray.length - 1].appendChild(nextList);
            }
            listParent = null;
            nested = false;
          }
    ...
    detachRangeFormatElement: function (rangeElement, selectedFormats, newRangeElement, remove, notHistoryPush) {
      ...
      function appendNode(parent, insNode, sibling, originNode) {
        ...
        while (insChildren[0]) {
          c = insChildren[0];
          if (util.notTextNode(c) && !util.isBreak(c) && !util.isListCell(format)) {
    ...
    nodeChange: function (appendNode, styleArray, removeNodeArray, strictRemove) {
            ...
            for (let i = 0; i < checkAttrs.length; i++) {
              if (sNode.nodeType === 1) {
                ...
                const styleCheck = isRemoveNode
                  ? !!sNode.style[s]
                  : !!sNode.style[s] && !!appendNode.style[s] && sNode.style[s] === appendNode.style[s];
                const classCheck =
                  classReg === false
                    ? false
                    : isRemoveNode
      ...
      const getMaintainedNode = this.util_getMaintainedNode.bind(util, isRemoveAnchor, isSizeNode);
      const isMaintainedNode = this.util_isMaintainedNode.bind(util, isRemoveAnchor, isSizeNode);
      ...
      if (oneLine) {
        ...
        this.setCommonListStyle(end.ancestor || util.getFormatElement(end.container), null);
      }
    ...
    util_getMaintainedNode: function (isRemove, isSizeNode, element) {
      ...
      return (
        ...
        (!isSizeNode ? this.getParentElement(element, this.isSizeNode.bind(this)) : null)
      );
    },
        ...
        for (let i = 0, len = childNodes.length, vNode; i < len; i++) {
          ...
          if (!endPass && child === endContainer) {
            ...
            const afterNode = util.createTextNode(
              endContainer.nodeType === 1 ? "" : endContainer.substringData(endOffset, endContainer.length - endOffset)
            );
            const textNode = util.createTextNode(
            ...
            if (anchorNode) {
              anchorNode = anchorNode.cloneNode(false);
            } else if (isMaintainedNode(newInnerNode.parentNode) && !anchorNode) {
              newInnerNode = newInnerNode.cloneNode(false);
              pNode.appendChild(newInnerNode);
              nNodeArray.push(newInnerNode);
            }
            ...
            if (anchorNode && child.nodeType === 3) {
              if (getMaintainedNode(child)) {
                const ancestorAnchorNode = util.getParentElement(
                  ancestor,
                  function (current) {
                    return this.isMaintainedNode(current.parentNode) || current.parentNode === pNode;
                  }.bind(util)
                );
      ...
      (function recursionFunc(current, ancestor) {
        ...
        for (let i = 0, len = childNodes.length, vNode, cloneChild; i < len; i++) {
          ...
          if (passNode && !util.isBreak(child)) {
            ...
            if (isMaintainedNode(newInnerNode.parentNode) && !isMaintainedNode(childNode)) {
              newInnerNode = newInnerNode.cloneNode(false);
              pNode.appendChild(newInnerNode);
              nNodeArray.push(newInnerNode);
            }
    ...
    actionCall: function (command, display, target) {
      ...
      if (display) {
        if (/more/i.test(display) && target !== this.moreLayerActiveButton) {
          ...
          (this.menuTray[command] === null || target !== this.submenuActiveButton)
        ) {
          ...
          (this.menuTray[command] === null || target !== this.containerActiveButton)
        ) {
      ...
      switch (command) {
        ...
        case "save":
          if (typeof options.callBackSave === "function") {
            ...
            throw Error(
              "[ExploreEditor.core.commandHandler.fail] Please register call back function in creation option. (callBackSave : Function)"
            );
          }
    ...
    toggleDisplayBlocks: function () {
      const wysiwyg = context.element.wysiwyg;
      util.toggleClass(wysiwyg, "meta-show-block");
      if (util.hasClass(wysiwyg, "meta-show-block")) {
    ...
    toggleCodeView: function () {
      ...
      if (isCodeView) {
        ...
        this._variable.codeOriginCssText = this._variable.codeOriginCssText.replace(
          /(\s?display(\s+)?:(\s+)?)[a-zA-Z]+(?=;)/,
          "display: none"
        );
        this._variable.wysiwygOriginCssText = this._variable.wysiwygOriginCssText.replace(
          /(\s?display(\s+)?:(\s+)?)[a-zA-Z]+(?=;)/,
          "display: block"
        );
        ...
        if (options.height === "auto" && !options.codeMirrorEditor) {
          context.element.code.style.height =
            context.element.code.scrollHeight > 0 ? context.element.code.scrollHeight + "px" : "auto";
        }
    ...
    setCodeDataToEditor: function () {
      ...
      if (options.fullPage) {
        ...
        this._iframeDocument.head.innerHTML = parseDocument.head.innerHTML;
        this._iframeDocument.body.innerHTML = this.convertContentsForEditor(parseDocument.body.innerHTML);
        ...
        for (let i = 0, len = attrs.length; i < len; i++) {
          ...
          this._iframeDocument.body.setAttribute(attrs[i].name, attrs[i].value);
        }
        if (!util.hasClass(this._iframeDocument.body, "meta-explore-editor-editable")) {
          const editableClasses = options.editableClass.split(" ");
          for (let i = 0; i < editableClasses.length; i++) {
            util.addClass(this._iframeDocument.body, options.editableClass[i]);
          }
        }
      } else {
    ...
    setEditorDataToCodeView: function () {
      ...
      if (options.fullPage) {
        const attrs = util.getAttributesToString(this._iframeDocument.body, null);
        codeValue =
          "<!DOCTYPE html>\n<html>\n" +
    ...
    toggleFullScreen: function (element) {
      ...
      if (!_var.isFullScreen) {
        ...
        if (context.element.stickyDummy.style.display !== ("none" && "")) {
          ...
          util.removeClass(toolbar, "meta-toolbar-sticky");
        }
        ...
        _var.innerHeight_fullScreen = _window.innerHeight - toolbar.offsetHeight;
        ...
        if (_var.fullScreenAttrs.sticky && !options.toolbarContainer) {
          ...
          util.addClass(toolbar, "meta-toolbar-sticky");
        }
        ...
        if (options.toolbarContainer) {
          util.removeClass(toolbar, "meta-toolbar-balloon");
        }
    ...
    print: function () {
      ...
      const contentsHTML = options.printTemplate
        ? options.printTemplate.replace(/\{\{\s*contents\s*\}\}/i, this.getContents(true))
        : this.getContents(true);
      const printDocument = util.getIframeDocument(iframe);
      const wDoc = this._iframeDocument;
      ...
      if (options.iframe) {
        ...
        printDocument.write(
          "" +
            ...
            "</head>" +
            '<body class="' +
            (options.printClass !== null ? options.printClass : options.editableClass) +
            '">' +
            contentsHTML +
            "</body>" +
            "</html>"
      ...
      _window.setTimeout(function () {
        try {
          iframe.focus();
          // IE or Edge
          if (util.isIE_Edge || !!_document.documentMode || !!_window.StyleMedia) {
            try {
          ...
          throw Error("[ExploreEditor.core.print.fail] error: " + error);
        } finally {
          core.closeLoading();
    ...
    preview: function (e) {
      ...
      const contentsHTML = options.previewTemplate
        ? options.previewTemplate.replace(/\{\{\s*contents\s*\}\}/i, this.getContents(true))
        : this.getContents(true);
    ...
    setIframeContents: function (ctx) {
      ...
      if (ctx.head) {
        this._iframeDocument.head.innerHTML = ctx.head.replace(/<script[\s\S]*>[\s\S]*<\/script>/gi, "");
      }
      if (ctx.body) {
        this._iframeDocument.body.innerHTML = this.convertContentsForEditor(ctx.body);
      }
    },
    ...
    getContents: function (onlyContents) {
      ...
      if (options.fullPage && !onlyContents) {
        const attrs = util.getAttributesToString(this._iframeDocument.body, ["contenteditable"]);
        return (
          "<!DOCTYPE html><html>" +
          this._iframeDocument.head.outerHTML +
          "<body " +
          attrs +
          ">" +
    ...
    makeLine: function (node, requireFormat) {
      ...
      if (node.nodeType === 3) {
        ...
        for (let i = 0, tLen = textArray.length, text; i < tLen; i++) {
          ...
          if (text.length > 0) {
            html += "<" + defaultTag + ">" + util.HTMLConvertor(text) + "</" + defaultTag + ">";
          }
        }
        return html;
      }
    ...
    cleanTags: function (rowLevelCheck, m, t) {
      ...
      let v = null;
      const tAttr = this.attributesTagsWhitelist[t.match(/(?!<)[a-zA-Z0-9\-]+/)[0].toLowerCase()];
      if (tAttr) {
        v = m.match(tAttr);
      } else {
      ...
      if (v) {
        for (let i = 0, len = v.length; i < len; i++) {
          if (!rowLevelCheck && /^class="(?!(__meta__|meta-|katex))/.test(v[i])) {
            continue;
          }
    ...
    cleanHTML: function (html, whitelist) {
      html = this.deleteDisallowedTags(this._parser.parseFromString(html, "text/html").body.innerHTML).replace(
        /(<[a-zA-Z0-9\-]+)[^>]*(?=>)/g,
      ...
      try {
        ...
        console.warn("[ExploreEditor.cleanHTML.consistencyCheck.fail] " + error);
      }
      ...
      for (let i = 0, len = domTree.length, t; i < len; i++) {
        t = domTree[i];
        if (t.nodeType === 1 && !util.isTextStyleElement(t) && !util.isBreak(t) && !util.disallowedTags(t)) {
          requireFormat = true;
          break;
        }
      }
      ...
      return this.tagConvertor(
        !cleanHTML
          ...
          : !whitelist
          ? cleanHTML
          : cleanHTML.replace(typeof whitelist === "string" ? util.createTagsWhitelist(whitelist) : whitelist, "")
      );
    },
    ...
    convertContentsForEditor: function (contents) {
      contents = this.deleteDisallowedTags(this._parser.parseFromString(contents, "text/html").body.innerHTML).replace(
        /(<[a-zA-Z0-9\-]+)[^>]*(?=>)/g,
      ...
      try {
        ...
        console.warn("[ExploreEditor.convertContentsForEditor.consistencyCheck.fail] " + error);
      }
    ...
    convertHTMLForCodeView: function (html) {
      ...
      const wDoc = typeof html === "string" ? _document.createRange().createContextualFragment(html) : html;
      ...
      indentSize = indentSize > 0 ? new _window.Array(indentSize + 1).join(" ") : "";
    ...
    addDocEvent: function (type, listener, useCapture) {
      ...
      if (options.iframe) {
        this._iframeDocument.addEventListener(type, listener);
      }
    },
    ...
    removeDocEvent: function (type, listener) {
      ...
      if (options.iframe) {
        this._iframeDocument.removeEventListener(type, listener);
      }
    },
    ...
    callCounterBlink: function () {
      ...
      if (charWrapper && !util.hasClass(charWrapper, "meta-blink")) {
        util.addClass(charWrapper, "meta-blink");
        _window.setTimeout(function () {
          util.removeClass(charWrapper, "meta-blink");
        }, 600);
      }
    },
    ...
    init: function (reload, initHTML) {
      const wRegExp = _window.RegExp;
      this._iframeWindow = options.iframe ? context.element.wysiwygFrame.contentWindow : _window;
      this._iframeDocument = _document;
      ...
      this.disallowedTextTagsRegExp =
        disallowTextTags.length === 0
          ? null
          : new wRegExp("(<\\/?)(" + disallowTextTags.join("|") + ")\\b\\s*([^>^<]+)?\\s*(?=>)", "gi");
      ...
      this.htmlCheckWhitelistRegExp = new wRegExp("^(" + options.editorTagsWhitelist.replace("|//", "") + ")$", "i");
      ...
      this.attributesWhitelistRegExp = new wRegExp("(?:" + allAttr + defaultAttr + ")" + regEndStr, "ig");
      ...
      this.fileManager.regExp = new wRegExp("^(" + this.fileManager.tags.join("|") + ")$", "i");
      ...
      // Init, validate
      if (options.iframe) {
        this._iframeDocument = context.element.wysiwygFrame.contentDocument;
        context.element.wysiwyg = this._iframeDocument.body;
        if (options.editorStyles.editor) {
          context.element.wysiwyg.style.cssText = options.editorStyles.editor;
        }

        if (options.height === "auto") {
          this.iframeAuto = this._iframeDocument.body;
        }
      }
    ...
    cachingButtons: function () {
      _window.setTimeout(
        function () {
          this.codeViewDisabledButtons = context.element.buttonTray.querySelectorAll(
            '.meta-menu-list button[data-display]:not([class~="meta-code-view-enabled"])'
          );
          this.resizingDisabledButtons = context.element.buttonTray.querySelectorAll(
            '.meta-menu-list button[data-display]:not([class~="meta-resizing-enabled"]):not([data-display="MORE"])'
          );
        }.bind(this)
      );
    ...
    initWysiwygArea: function (reload, initHTML) {
      context.element.wysiwyg.innerHTML = reload
        ...
        : this.convertContentsForEditor(
            typeof initHTML === "string" ? initHTML : context.element.originElement.value || ""
          );
    },
    ...
    iframeAutoHeight: function () {
      if (this.iframeAuto) {
        _window.setTimeout(function () {
          context.element.wysiwygFrame.style.height = this.iframeAuto.offsetHeight + "px";
        });
      }
    },
    ...
    setOptionsInit: function (el, initHTML) {
      this.context = context = $context(el.originElement, this.getConstructed(el), options);
  ...
  const event = {
    ...
    nonTextKeyCode: new _window.RegExp("^(8|13|1[6-9]|20|27|3[3-9]|40|45|46|11[2-9]|12[0-3]|144|145)$"),
    historyIgnoreKeyCode: new _window.RegExp("^(1[6-9]|20|27|3[3-9]|40|45|11[2-9]|12[0-3]|144|145)$"),
    onButtonsCheck: new _window.RegExp("^(" + _window.Object.keys(options.textTagsMap).join("|") + ")$", "i"),
        ...
        if (util.isFormatElement(element)) {
          ...
          if (commandMapNodes.indexOf("OUTDENT") === -1 && commandMap.OUTDENT) {
            if (
              ...
              (element.style[marginDir] && util.getNumber(element.style[marginDir], 0) > 0)
            ) {
    ...
    buttonsEventHandler: function (e) {
      ...
      if (util.getParentElement(target, ".meta-submenu")) {
        e.stopPropagation();
        ...
        while (!command && !/meta-menu-list/.test(className) && !/meta-common/.test(className)) {
    ...
    onClick_toolbar: function (e) {
      ...
      while (target.parentNode && !command && !/meta-menu-list/.test(className) && !/meta-toolbar/.test(className)) {
        target = target.parentNode;
        command = target.getAttribute("data-command");
    ...
    onMouseDown_wysiwyg: function (e) {
      ...
      if (tableCell) {
        ...
        if (tablePlugin && tableCell !== tablePlugin.fixedCell && !tablePlugin.shift) {
          core.callPlugin(
            "table",
            function () {
      ...
      if ((!formatEl || formatEl === rangeEl) && !util.isNonEditable(targetElement) && !util.isList(rangeEl)) {
        ...
        if (util.getFormatElement(range.startContainer) === util.getFormatElement(range.endContainer)) {
          if (util.isList(rangeEl)) {
            ...
            e.preventDefault();
            core.setDefaultFormat(util.isRangeFormatElement(rangeEl) ? "DIV" : options.defaultTag);
            core.focus();
          } else {
            event.applyTagEffects();
          }
    ...
    toggleToolbarBalloon: function () {
      core.editorRange();
      const range = core.getRange();
      if (core.bindControllersOff || (!core.isBalloonAlways && range.collapsed)) {
        event.hideToolbar();
      } else {
    ...
    showToolbarBalloon: function (rangeObj) {
      ...
      if (core.isBalloonAlways && range.collapsed) {
        ...
        isDirTop =
          util.getArrayIndex(childNodes, selection.focusNode) < util.getArrayIndex(childNodes, selection.anchorNode);
      }
      ...
      const arrowMargin = _window.Math.round(context.element.arrow.offsetWidth / 2);
    ...
    setToolbarOffset: function (
      ...
      let resetTop = false;
      const space =
        t + (isDirTop ? event.getEditorOffsets(null).top : toolbar.offsetHeight - context.element.wysiwyg.offsetHeight);
      ...
      if (isDirTop) {
        util.removeClass(context.element.arrow, "meta-arrow-up");
        util.addClass(context.element.arrow, "meta-arrow-down");
        context.element.arrow.style.top = toolbarHeight + "px";
      } else {
        util.removeClass(context.element.arrow, "meta-arrow-down");
        util.addClass(context.element.arrow, "meta-arrow-up");
        context.element.arrow.style.top = -arrowMargin + "px";
      }

      const arrow_left = _window.Math.floor(toolbarWidth / 2 + (absoluteLeft - l));
    ...
    isUneditableNode: function (range, isFront) {
      ...
      if (isElement) {
        siblingNode = event.isUneditableNode_getSibling(container.childNodes[offset], siblingKey, container);
        return siblingNode && siblingNode.nodeType === 1 && siblingNode.getAttribute("contenteditable") === "false";
      } else {
        siblingNode = event.isUneditableNode_getSibling(container, siblingKey, container);
        return (
    ...
    onKeyDown_wysiwyg: function (e) {
      ...
      const fileComponentName = core.fileManager.pluginRegExp.test(core.currentControllerName)
      ...
      switch (keyCode) {
        case 8 /** backspace key */:
          ...
          if (rangeEl && formatEl && !util.isCell(rangeEl) && !/^FIGCAPTION$/i.test(rangeEl.nodeName)) {
            if (
              ...
              (selectionNode === formatEl ||
                (selectionNode.nodeType === 3 &&
                  (!selectionNode.previousSibling || util.isList(selectionNode.previousSibling)))) &&
              (util.getFormatElement(range.startContainer, null) !== util.getFormatElement(range.endContainer, null)
          ...
          if (
            ...
            if (
              !sel.previousSibling &&
              (util.isComponent(commonCon.previousSibling) || (ignoreZWS && util.isComponent(prev)))
            ) {
        ...
        case 46 /** delete key */:
          ...
          if (
            (util.isFormatElement(selectionNode) ||
              ...
              (util.onlyZeroWidthSpace(selectionNode.nextSibling) && selectionNode.nextSibling.nextSibling === null)) &&
          ...
          if (
            ...
            next = next || formatEl.nextElementSibling || rElParent.nextElementSibling;
        ...
        case 9 /** tab key */:
          ...
          if (lines.length > 0) {
            if (!shift) {
              const tabText = util.createTextNode(new _window.Array(core._variable.tabSize + 1).join("\u00A0"));
              if (lines.length === 1) {
        ...
        case 13 /** enter key */:
          ...
          if (!shift) {
            const formatInners = core.isEdgeFormat(range.endContainer, range.endOffset, "end");
            ...
            if (freeFormatEl) {
              ...
              if (
                ...
                ((selectionFormat &&
                  ...
                  (!children[offset + 1] ||
                    ((!children[offset + 2] || util.onlyZeroWidthSpace(children[offset + 2].textContent)) &&
                ...
                const newEl = core.appendFormatTag(
                  freeFormatEl,
                  util.isFormatElement(freeFormatEl.nextElementSibling) ? freeFormatEl.nextElementSibling : null
                );
              ...
              if (selectionFormat) {
                functions.insertHTML(
                  range.collapsed && util.isBreak(range.startContainer.childNodes[range.startOffset - 1])
                    ? "<br>"
                    : "<br><br>",
                  true,
                  false
          ...
          if (fileComponentName) {
            ...
            core.callPlugin(
              fileComponentName,
              function () {
                if (core.selectComponent(compContext.element, fileComponentName) === false) {
                  core.blur();
                }
              },
              null
      ...
      const textKey = !ctrl && !alt && !selectRange && !event.nonTextKeyCode.test(keyCode);
      if (
        textKey &&
        range.collapsed &&
        range.startContainer === range.endContainer &&
    ...
    onKeyUp_wysiwyg: function (e) {
      ...
      if (core.isBalloon && ((core.isBalloonAlways && keyCode !== 27) || !range.collapsed)) {
      ...
      if (
        ...
        const oFormatTag = util.createElement(
          util.isFormatElement(core._variable.currentNodes[0]) ? core._variable.currentNodes[0] : options.defaultTag
        );
      ...
      if (
        ...
        let so = range.startOffset,
          eo = range.endOffset;
        const frontZeroWidthCnt = (selectionNode.textContent.substring(0, eo).match(event.frontZeroWidthReg) || "")
          .length;
    ...
    resize_editor: function (e) {
      ...
      context.element.wysiwygFrame.style.height = context.element.code.style.height =
        (resizeInterval < core._variable.minResizingSize ? core._variable.minResizingSize : resizeInterval) + "px";
      core._variable.resizeClientY = e.clientY;
    },
    ...
    onResize_window: function (e) {
      ...
      if (responsiveSize) {
        let w = 0;
        if ((core.isBalloon || core.isInline) && options.toolbarWidth === "auto") {
      ...
      if (context.fileBrowser && context.fileBrowser.area.style.display === "block") {
        context.fileBrowser.body.style.maxHeight =
          _window.innerHeight - context.fileBrowser.header.offsetHeight - 50 + "px";
      }
      ...
      if (core._variable.isFullScreen) {
        core._variable.innerHeight_fullScreen +=
          _window.innerHeight - context.element.toolbar.offsetHeight - core._variable.innerHeight_fullScreen;
    ...
    onScroll_window: function () {
      ...
      const y = (this.scrollY || _document.documentElement.scrollTop) + options.stickyToolbar;
      const editorTop =
        event.getEditorOffsets(options.toolbarContainer).top - (core.isInline ? element.toolbar.offsetHeight : 0);
      const inlineOffset =
        core.isInline && y - editorTop > 0 ? y - editorTop - context.element.toolbar.offsetHeight : 0;
      ...
      if (y < editorTop) {
        ...
        element.toolbar.style.top =
          inlineOffset + editorHeight + editorTop + options.stickyToolbar - y - core._variable.minResizingSize + "px";
      } else if (y >= editorTop) {
        event.onStickyToolbar(inlineOffset);
      }
    },
    ...
    getPageBottomSpace: function () {
      return (
        ...
        (event.getEditorOffsets(null).top + context.element.topArea.offsetHeight)
      );
    },
    ...
    onStickyToolbar: function (inlineOffset) {
      ...
      element.toolbar.style.width = core.isInline ? core.inlineToolbarAttr.width : element.toolbar.offsetWidth + "px";
      util.addClass(element.toolbar, "meta-toolbar-sticky");
      core.sticky = true;
    },
    ...
    offStickyToolbar: function () {
      ...
      element.toolbar.style.top = core.isInline ? core.inlineToolbarAttr.top : "";
      element.toolbar.style.width = core.isInline ? core.inlineToolbarAttr.width : "";
      element.editorArea.style.marginTop = "";

      util.removeClass(element.toolbar, "meta-toolbar-sticky");
      core.sticky = false;
    },
    ...
    hardDelete: function () {
      ...
      // component
      const sComp = sc.nodeType === 1 ? util.getParentElement(sc, ".meta-component") : null;
      const eComp = ec.nodeType === 1 ? util.getParentElement(ec, ".meta-component") : null;
      if (sComp) {
    ...
    onCopy_wysiwyg: function (e) {
      ...
      if (info && !util.isIE) {
        ...
        util.addClass(info.component, "meta-component-copy");
        // copy effect
        _window.setTimeout(function () {
          util.removeClass(info.component, "meta-component-copy");
        }, 150);
      }
    },
    ...
    setDropLocationSelection: function (e) {
      ...
      } else if (core._iframeDocument.caretRangeFromPoint) {
        const r = core._iframeDocument.caretRangeFromPoint(e.clientX, e.clientY);
    ...
    dataTransferAction: function (type, e, data) {
      ...
      if (util.isIE) {
        ...
        if (event.setClipboardData(type, e, plainText, cleanData, data) === false) {
          e.preventDefault();
          e.stopPropagation();
          return false;
        }
    ...
    setClipboardData: function (type, e, plainText, cleanData, data) {
      ...
      const maxCharCount = core.charCount(core.charTypeHTML ? cleanData : plainText);
      // user event - paste
    ...
    onMouseMove_wysiwyg: function (e) {
      ...
      if (component && !core.currentControllerName) {
        ...
        let dir = "",
          top;
        if (
          (isList ? !component.previousSibling : !util.isFormatElement(component.previousElementSibling)) &&
          y < c + 20
        ) {
          top = componentTop;
          dir = "t";
    ...
    onLineBreak: function (e) {
      ...
      if (core.charTypeHTML && !core.checkCharCount(format.outerHTML, "byte-html")) {
        return;
      }
    ...
    addEvent: function () {
      const eventWysiwyg = options.iframe ? core._iframeWindow : context.element.wysiwyg;

      /** toolbar event */
      context.element.toolbar.addEventListener("mousedown", event.buttonsEventHandler, false);
      context.element.menuTray.addEventListener("mousedown", event.buttonsEventHandler, false);
      ...
      core.lineBreakerButton.addEventListener("mousedown", event.onMouseDown_lineBreak, false);
      ...
      if (options.height === "auto" && !options.codeMirrorEditor) {
        context.element.code.addEventListener("keydown", event.codeViewAutoHeight, false);
        context.element.code.addEventListener("keyup", event.codeViewAutoHeight, false);
        context.element.code.addEventListener("paste", event.codeViewAutoHeight, false);
      }
      ...
      if (context.element.resizingBar) {
        if (/\d+/.test(options.height)) {
          ...
          util.addClass(context.element.resizingBar, "meta-resizing-none");
        }
      }
    ...
    removeEvent: function () {
      ...
      context.element.toolbar.removeEventListener("mousedown", event.buttonsEventHandler);
      context.element.menuTray.removeEventListener("mousedown", event.buttonsEventHandler);
      ...
      core.lineBreakerButton.removeEventListener("mousedown", event.onMouseDown_lineBreak);
      ...
      context.element.code.removeEventListener("keydown", event.codeViewAutoHeight);
      context.element.code.removeEventListener("keyup", event.codeViewAutoHeight);
      context.element.code.removeEventListener("paste", event.codeViewAutoHeight);
    ...
    setResponsiveToolbar: function () {
      ...
      for (let i = 1, len = responsiveButtons.length, size, buttonGroup; i < len; i++) {
  ...
  const functions = {
    ...
    setToolbarButtons: function (toolbarItem) {
      ...
      const newToolbar = $constructor.createToolBar(toolbarItem, core.plugins, options);
      ...
      const newContext = $context(context.element.originElement, core.getConstructed(context.element), options);

      context.element = newContext.element;
      context.tool = newContext.tool;
      if (options.iframe) {
        context.element.wysiwyg = core._iframeDocument.body;
      }
      core.cachingButtons();
      ...
      if (util.hasClass(context.element.wysiwyg, "meta-show-block")) {
    ...
    setOptions: function (_options) {
      ...
      // set option
      const consOptions = $constructor.setOptions(mergeOptions, context, options);
    ...
    setDefaultStyle: function (style) {
      const newStyles = (options.editorStyles = util.setDefaultOptionStyle(options, style));
      const el = context.element;
    ...
    /**
     * @description Gets the ExploreEditor's context object. Contains settings, plugins, and cached element objects
     * @returns {Object}
     */
    getContext: function () {
      return context;
    ...
    /**
     * @description Gets the contents of the ExploreEditor
     * @param {Boolean} onlyContents - Return only the contents of the body without headers when the "fullPage" option is true
     * @returns {String}
     */
    getContents: function (onlyContents) {
    ...
    /**
     * @description Gets only the text of the ExploreEditor contents
     * @returns {String}
     */
    getText: function () {
    ...
    getCharCount: function (charCounterType) {
      ...
      return core.getCharLength(
        core.charTypeHTML ? context.element.wysiwyg.innerHTML : context.element.wysiwyg.textContent,
        charCounterType
      );
    },
    ...
    insertHTML: function (html, notCleaningData, checkCharCount, rangeSelection) {
      if (typeof html === "string") {
        ...
        try {
          ...
          console.warn("[ExploreEditor.insertHTML.fail] " + error);
    ...
    /**
     * @description Change the contents of the ExploreEditor
    ...
    /**
     * @description Add contents to the ExploreEditor
    ...
    /**
     * @description Disable the ExploreEditor
     */
    disabled: function () {
    ...
    /**
     * @description Enable the ExploreEditor
     */
    enabled: function () {
    ...
    /**
     * @description Show the ExploreEditor
     */
    show: function () {
    ...
    /**
     * @description Hide the ExploreEditor
     */
    hide: function () {
    ...
    /**
     * @description Destroy the ExploreEditor
     */
    destroy: function (e) {
      /** off menus */
      core.submenuOff();
