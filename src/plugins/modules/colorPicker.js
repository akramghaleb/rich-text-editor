...
export default {
  ...
  add: function (core) {
    ...
    /** set submenu */
    context.colorPicker.colorListHTML = this.createColorList(core, this.makeColorList);
  },
  ...
  createColorList: function (core, makeColor) {
    ...
    let colorArr = [];
    let list = '<div class="meta-list-inner">';
    for (let i = 0, len = colorList.length, color; i < len; i++) {
      ...
      if (colorArr.length > 0) {
        list += '<div class="meta-selector-color">' + makeColor(colorArr) + "</div>";
        colorArr = [];
      }
      if (typeof color === "object") {
        list += '<div class="meta-selector-color">' + makeColor(color) + "</div>";
      }
    }
    list +=
      "" +
      '<form class="meta-submenu-form-group">' +
      '<input type="text" maxlength="9" class="_meta_color_picker_input meta-color-input"/>' +
      '<button type="submit" class="meta-btn-primary _meta_color_picker_submit" title="' +
      lang.dialogBox.submitButton +
      '">' +
      core.icons.checked +
      "</button>" +
      '<button type="button" class="meta-btn _meta_color_picker_remove" title="' +
  ...
  makeColorList: function (colorList) {
    ...
    list += '<ul class="meta-color-pallet">';
    for (let i = 0, len = colorList.length, color; i < len; i++) {
      color = colorList[i];
      if (typeof color === "string") {
        list +=
  ...
  init: function (node, color) {
    ...
    let fillColor = color
      ? color
      : colorPicker.getColorInNode.call(this, node) || this.context.colorPicker.defaultColor;
