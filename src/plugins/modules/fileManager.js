...
export default {
    ...
    if (xmlHttp.readyState === 4) {
      if (xmlHttp.status === 200) {
        try {
          ...
          throw Error('[ExploreEditor.fileManager.upload.callBack.fail] cause : "' + e.message + '"');
        } finally {
          this.closeLoading();
        }
      } else {
        // exception
        this.closeLoading();
        let xhresult = this.util.isJsonStr(xmlHttp.responseText)
          ? JSON.parse(xmlHttp.responseText)
          : xmlHttp.responseText;
        const res = !xmlHttp.responseText ? xmlHttp : xhresult;
        if (typeof errorCallBack !== "function" || errorCallBack("", res, this)) {
          const err =
            "[ExploreEditor.fileManager.upload.serverException] status: " +
            xmlHttp.status +
            ", response: " +
  ...
  checkInfo: function (pluginName, tagNames, uploadEventHandler, modifyHandler, resizing) {
    ...
    for (let i = 0, len = tags.length, tag; i < len; i++) {
      tag = tags[i];
      if (!this.util.getParentElement(tag, this.util.isMediaComponent) || !fileManagerPlugin.checkMediaComponent(tag)) {
  ...
  setInfo: function (pluginName, element, uploadEventHandler, file, resizing) {
    ...
    if (resizing) {
      ...
      if (!element.getAttribute("data-origin")) {
        ...
        const w = this.plugins.resizing.module_getSizeX.call(this, context, element, cover, container);
        const h = this.plugins.resizing.module_getSizeY.call(this, context, element, cover, container);
