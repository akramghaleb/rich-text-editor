...
export default {
  ...
  add: function (core) {
    ...
    contextAudio.audioInputFile = audio_dialog.querySelector("._meta_audio_files");
    contextAudio.audioUrlFile = audio_dialog.querySelector(".meta-input-url");
    contextAudio.focusElement = contextAudio.audioInputFile || contextAudio.audioUrlFile;
    contextAudio.preview = audio_dialog.querySelector(".meta-link-preview");
    ...
    if (contextAudio.audioInputFile) {
      audio_dialog
        ...
        .addEventListener(
          "click",
          this.removeSelectedFiles.bind(contextAudio.audioInputFile, contextAudio.audioUrlFile, contextAudio.preview)
        );
    }
    if (contextAudio.audioInputFile && contextAudio.audioUrlFile) {
      contextAudio.audioInputFile.addEventListener("change", this.fileInputChange.bind(contextAudio));
    }
    ...
    if (contextAudio.audioUrlFile) {
      contextAudio.audioUrlFile.addEventListener(
        "input",
        this.onLinkPreview.bind(contextAudio.preview, contextAudio, core.options.linkProtocol)
      );
    }
  ...
  setDialog: function (core) {
    ...
    if (option.audioFileInput) {
      html +=
        ...
        '<input class="meta-input-form _meta_audio_files" type="file" accept="' +
        option.audioAccept +
        '"' +
  ...
  setController: function (core) {
    ...
    link_btn.className = "meta-controller meta-controller-link";
    link_btn.innerHTML =
      "" +
      '<div class="meta-arrow meta-arrow-up"></div>' +
      '<div class="link-content">' +
      '<div class="meta-btn-group">' +
      '<button type="button" data-command="update" tabindex="-1" class="meta-tooltip">' +
      icons.edit +
  ...
  submit: function (e) {
    ...
    try {
      ...
      } else if (contextAudio.audioUrlFile && contextAudio.linkValue.length > 0) {
        this.showLoading();
        this.plugins.audio.setupUrl.call(this, contextAudio.linkValue);
      }
    } catch (error) {
      this.closeLoading();
      throw Error('[ExploreEditor.audio.submit.fail] cause : "' + error.message + '"');
    } finally {
  ...
  submitAction: function (fileList) {
    ...
    if (limitSize > 0) {
      ...
      if (fileSize + infoSize > limitSize) {
        this.closeLoading();
        const err = "[ExploreEditor.audioUpload.fail] Size of uploadable total audios: " + limitSize / 1000 + "KB";
        if (
  ...
  error: function (message, response) {
    ...
    if (
      ...
      throw Error("[ExploreEditor.plugin.audio.exception] response: " + message);
    }
  },
  ...
  setupUrl: function (src) {
    try {
      ...
      throw Error('[ExploreEditor.audio.audio.fail] cause : "' + error.message + '"');
    } finally {
      this.closeLoading();
    }
  },
  ...
  updateCover: function (element) {
    ...
    const cover = this.plugins.component.set_cover.call(this, element);
    const container = this.plugins.component.set_container.call(this, cover, "meta-audio-container");
    ...
    try {
      ...
      console.warn("[ExploreEditor.audio.error] Maybe the audio tag is nested.", error);
    }
  ...
  onModifyMode: function (selectionTag) {
    ...
    contextAudio.container = this.util.getParentElement(selectionTag, this.util.isComponent);
  },
  ...
  init: function () {
    ...
    if (contextAudio.audioUrlFile) {
      contextAudio.linkValue = contextAudio.preview.textContent = contextAudio.audioUrlFile.value = "";
    }
    if (contextAudio.audioInputFile && contextAudio.audioUrlFile) {
