...
export default {
  ...
  setSubmenu: function (core) {
    ...
    listDiv.innerHTML =
      ...
      '<li><button type="button" class="meta-btn-list meta-tooltip" data-command="OL" title="' +
      lang.toolbar.orderList +
      '">' +
      core.icons.list_number +
      "</button></li>" +
      '<li><button type="button" class="meta-btn-list meta-tooltip" data-command="UL" title="' +
  ...
  editList(command, selectedCells, detach) {
    ...
    if (
      ...
      (!bottomEl || lastSel.tagName !== bottomEl.tagName || command !== bottomEl.tagName.toUpperCase())
    ) {
      if (detach) {
        for (let i = 0, len = selectedFormats.length; i < len; i++) {
          for (let j = i - 1; j >= 0; j--) {
    ...
    if (remove) {
      ...
      for (let i = 0, len = cellsLen, c; i < len; i++) {
        ...
        if (c.parentNode !== originList) {
          this.plugins.list.insiedList.call(this, originList, innerList, prev, next, nodePath);
