...
export default {
  ...
  setSubmenu: function (core) {
    ...
    for (let i = 0, len = formatList.length, format, tagName, command, name, h, attrs, className; i < len; i++) {
      ...
      list +=
        "<li>" +
        '<button type="button" class="meta-btn-list" data-command="' +
        command +
        '" data-value="' +
        tagName +
        '" data-class="' +
  ...
  active: function (element) {
    ...
    if (!element) {
      ...
      const className = (element.className.match(/(\s|^)__meta__format__[^\s]+/) || [""])[0].trim();
        ...
        for (let i = 0, len = modifiedFormsts.length, node, newFormat; i < len; i++) {
          ...
          if (
            (node.nodeName.toLowerCase() !== value.toLowerCase() ||
              (node.className.match(/(\s|^)__meta__format__[^\s]+/) || [""])[0].trim() !== className) &&
