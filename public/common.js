...
function setVideoList() {
  ...
  for (let i = 0, video; i < videoList.length; i++) {
    ...
    list +=
      "<li>" +
      '<button title="delete" onclick="selectVideo(\'delete\',' +
      video.index +
      ')">X</button>' +
      '<a href="javascript:void(0)" onclick="selectVideo(\'select\',' +
      video.index +
      ')">' +
      video.src +
      "</a>" +
...
function setImageList() {
  ...
  for (let i = 0, image, fixSize; i < imageList.length; i++) {
    ...
    list +=
      ...
      '"></div>' +
      "</div>" +
      '<a href="javascript:void(0)" onclick="selectImage(\'select\',' +
      image.index +
      ')" class="image-size">' +
      fixSize +
      "KB</a>" +
...
function JSONstringify(json, lang) {
  ...
  json = Object.assign(
    ...
    { lang: "ExploreEditor_LANG." + lang, "lang(In nodejs)": lang },
    codeMirror,
    katex
  );
